...
# Paths
INPUT_CSV = "movXArticuloConDetalle.csv"
...
def main():
    ...
    for col in df_filtered.select_dtypes(include=["object", "string"]).columns:
        df_filtered[col] = (
            df_filtered[col]
            .astype(str)
            .str.replace(",", " -", regex=False)   # replace comma with dash or whatever you want
            .str.strip()
        )
    ...
    # --- 3. LOAD (to CSV) ---
    df_filtered.to_csv(OUTPUT_CSV, index=False, encoding='utf-8-sig', sep=',')#, quoting=csv.QUOTE_ALL)
